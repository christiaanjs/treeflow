...
| --- | --- | --- |
| `fixed` | Uses the supplied input tree as-is (no tree prior). | *(none)* |
...
| Name | Description | Parameters |
| --- | --- | --- |
| `none` (default) | A single site rate; no rate heterogeneity across sites. | *(none)* |
...
| Name | Description | Parameters |
| --- | --- | --- |
| `jc` | Jukes–Cantor. Base frequencies are fixed to uniform. | *(none)* |
...
models that can be built with TreeFlow's Python API. In particular, the `kappa` and other
substitution parameters in the YAML format are scalars shared across the whole tree.

Richer models — for example a **per-branch ("relaxed") transition/transversion ratio**,
where each branch is given its own independent `kappa` — are built with the Python API by
broadcasting a per-branch parameter tensor through the standard substitution machinery.
This is demonstrated in the
[`examples/carnivores.ipynb`](https://github.com/christiaanjs/treeflow/blob/master/examples/carnivores.ipynb)
notebook, which draws `kappa` with shape `(..., n_branches)` (e.g. via
