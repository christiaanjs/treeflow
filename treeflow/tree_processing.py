...
def parse_newick(newick_file):
    ...
    return {
        "topology": {"parent_indices": parent_indices},
        "heights": heights,
    }, taxon_names
...
def update_topology_dict(topology):
    ...
    return dict(
        postorder_node_indices=get_postorder_node_indices(
            len(parent_indices) // 2 + 1
        ),  # Parent indices for every vertex except root
...
TreeInfo = collections.namedtuple(
    "TreeInfo", ["tree", "node_bounds"]
)  # TODO: Think about proper object model for tree
...
def parse_tree_info(newick_file):
    ...
    return TreeInfo(
        tree,
        get_node_anchor_heights(
            tree["heights"],
            get_postorder_node_indices(len(taxon_names)),
...
def tree_to_tensor(tree):
    return {
        ...
        "topology": {
            ...
            for key, value in tree["topology"].items()
        },
    }
