...
class BirthDeath(treeflow.tree.TreeDistribution):
    def __init__(
        ...
        birth_diff_rate,
        relative_death_rate,
        sample_probability=tf.convert_to_tensor(1.0, dtype=DEFAULT_FLOAT_DTYPE_TF),
        validate_args=False,
    ...
    def _log_prob_1d(self, x, r, a, rho):
        ...
        log_coeff = (taxon_count - 1) * tf.math.log(
            tf.convert_to_tensor(2.0, dtype=DEFAULT_FLOAT_DTYPE_TF)
    ...
    def _log_prob(self, x):  # For now we assume that parameters aren't batched
        ...
        return treeflow.tf_util.vectorize_1d_if_needed(
            self._log_prob_1d_flat, x_flat, batch_shape.rank
        )
