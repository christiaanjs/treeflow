...
class ParentCorrelation(tfp.bijectors.ScaleMatvecLU):
    def __init__(self, parent_indices, beta, name="ParentAffine"):
        ...
        indices = tf.concat(
            [
                tf.expand_dims(tf.range(non_root_count), 1),
                tf.expand_dims(tf.convert_to_tensor(parent_indices, tf.int32), 1),
            ],
            axis=1,
        )
        build_triu = lambda beta: tf.eye(node_count) + tf.scatter_nd(
            indices, beta, [node_count, node_count]
        )
        super(ParentCorrelation, self).__init__(
            tfp.util.DeferredTensor(beta, build_triu, shape=[node_count, node_count]),
            perm,
            name=name,
        )
...
class BranchBreaking(tfp.bijectors.Bijector):  # TODO: Broadcast over batch_dims
    def __init__(
        ...
        self.anchor_heights = (
            tf.zeros(len(preorder_node_indices) + 1, dtype=DEFAULT_FLOAT_DTYPE_TF)
    ...
    def _forward_1d(self, x):
        ...
        init = tf.scatter_nd(
            ...
            self.anchor_heights.shape,
        )

        def f(out, elems):
            node_index, parent_index, proportion, anchor_height = elems
            node_height = (
                out[parent_index] - anchor_height
            ) * proportion + anchor_height
            return tf.tensor_scatter_nd_update(
                out, tf.reshape(node_index, [1, 1]), tf.expand_dims(node_height, 0)
            )
        ...
        return tf.scan(
            ...
            (
                ...
                tf.gather(x, self.preorder_node_indices),
                tf.gather(self.anchor_heights, self.preorder_node_indices),
            ),
            init,
        )[-1]
    ...
    def _forward(self, x):
        return treeflow.tf_util.vectorize_1d_if_needed(
            self._forward_1d, x, tf.shape(tf.shape(x))[0] - 1
        )

    def _inverse_1d(self, y):
        return (y - self.anchor_heights) / tf.concat(
            [
                (tf.gather(y, self.parent_indices) - self.anchor_heights[:-1]),
                tf.ones((1,), dtype=DEFAULT_FLOAT_DTYPE_TF),
            ],
            0,
        )
    ...
    def _inverse(self, y):
        return treeflow.tf_util.vectorize_1d_if_needed(
            self._inverse_1d, y, tf.shape(tf.shape(y))[0] - 1
        )

    def _inverse_log_det_jacobian_1d(self, y):
        return -tf.reduce_sum(
            tf.math.log(tf.gather(y, self.parent_indices) - self.anchor_heights[:-1])
        )
...
class Ratio(BranchBreaking):
    ...
    def _forward_1d_numpy(
        self, x
    ):  # TODO: Should we do vectorization in Numpy or TF land?
    ...
    def _forward_1d(self, x):
        ...
        def libsbn_tf_func(x):
            heights = tf.numpy_function(
                self._forward_1d_numpy, [x], DEFAULT_FLOAT_DTYPE_TF
            )

            def grad(dheights):
                return tf.numpy_function(
                    self._ratio_gradient_numpy,
                    [heights, dheights],
        ...
        with_root_height = tf.concat(
            [x[:-1], x[-1:] + self.anchor_heights[-1]], axis=0
...
class TreeChain(tfp.bijectors.Chain):
    def __init__(
        ...
        ratio_bijector = (
            BranchBreaking(
                parent_indices, preorder_node_indices, anchor_heights=anchor_heights
            )
            if inst is None
            else Ratio(
                inst,
        ...
        super(FixedLeafHeightDistribution, self).__init__(
            [
                tfp.distributions.Independent(
                    tfp.distributions.Deterministic(leaf_heights),
...
class FixedTopologyDistribution(tfp.distributions.JointDistributionNamed):
    def __init__(self, height_distribution, topology, name="FixedTopologyDistribution"):
        super(FixedTopologyDistribution, self).__init__(
            dict(
    ...
    def reparameterization_type(self):  # Hack to allow VI
        return dict(
            heights=self.heights_reparam,
            topology={
                key: tfp.distributions.FULLY_REPARAMETERIZED
