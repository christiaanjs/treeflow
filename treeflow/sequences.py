...
def log_prob_conditioned(value, topology, category_count):
    ...
    def log_prob(branch_lengths, subst_model, category_weights, category_rates, frequencies, **subst_model_params):
        ...
        @tf.custom_gradient
        def log_prob_flat(branch_lengths, category_weights, category_rates, frequencies, *subst_model_params_list):
            ...
            def grad(dlog_prob):
                ...
                return [dlog_prob * grad for grad in ([
                    likelihood.compute_branch_length_derivatives(q, category_rates, category_weights),
                    likelihood.compute_weight_derivatives(category_weights),
                    likelihood.compute_rate_derivatives(q, branch_lengths, category_weights),
                    tf.stack([likelihood.compute_frequency_derivative(freq_differentials[i], i, category_weights) for i in range(4)])
                ] + param_grads)]
...
class LeafSequences(tfp.distributions.Distribution):
    ...
    def _log_prob(self, value):
        return log_prob_conditioned(value, self.tree['topology'], len(self.category_weights))(self.tree['heights'], self.subst_model, self.category_weights, self.category_rates, self.frequencies, **self.subst_model_params)
