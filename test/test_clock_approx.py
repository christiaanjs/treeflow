        ...
        dict(
            topology=dict(
                parent_indices=tf.broadcast_to(
                    ...
                    [sample_shape, tree["topology"]["parent_indices"].shape[0]],
                )
            ),
            heights=tf.broadcast_to(
                tree["heights"], [sample_shape, tree["heights"].shape[0]]
            ),
        ),
        taxon_count,
    )
...
def test_rate_approx_sample_scalar(hello_tensor_tree_and_count):
    tree, taxon_count = hello_tensor_tree_and_count
    dist = treeflow.clock_approx.ScaledRateDistribution(
        tfp.distributions.Sample(lognormal_dist, 2 * taxon_count - 2), tree, clock_rate
    )
    sample = dist.sample()
...
def test_rate_approx_sample_vector(hello_tensor_tree_vec_and_count):
    ...
    base_rate_dist = lambda **x: tfp.distributions.Sample(
        tfp.distributions.LogNormal(**x), 2 * taxon_count - 2
    )
    rate_dist = treeflow.clock_approx.ScaledRateDistribution(
        base_rate_dist, tree, clock_vec, **lognormal_kwargs
    )
    rates = rate_dist.sample()
    assert rates.numpy().shape == (
        sample_shape,
        2 * taxon_count - 2,
    )
