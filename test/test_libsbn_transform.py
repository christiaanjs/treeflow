...
def get_transforms(newick_file_dated):
...
def test_libsbn_transform_forward(newick_file_dated):
    tf_transform, libsbn_transform, tree_info = get_transforms(newick_file_dated)
...
def test_libsbn_transform_gradient(newick_file_dated):
    tf_transform, libsbn_transform, tree_info = get_transforms(newick_file_dated)
    ratios = tf.convert_to_tensor(
        get_ratios(tree_info.node_bounds.shape[0] + 1), dtype=DEFAULT_FLOAT_DTYPE_TF
    )
