import pytest
from treeflow.substitution_model import (
    JC,
    HKY,
    normalising_constant,
    normalised_differential,
    transition_probs,
    transition_probs_expm,
...
def test_transition_probs_hky_rowsum(hky_params, branch_lengths, category_rates):
    transition_prob_vals = get_transition_probs_vals(
        JC(), branch_lengths, category_rates, **hky_params
    )
...
def test_transition_probs_jc(branch_lengths, category_rates):
    transition_prob_vals = get_transition_probs_vals(
        JC(), branch_lengths, category_rates
    )
    ...
    for rate_index, rate in enumerate(category_rates):
        for branch_index, branch_length in enumerate(branch_lengths):
            ...
            assert_allclose(
                0.25 + 0.75 * np.exp(-4 * d / 3), probs[diag_i, diag_j], rtol=1e-6
            )
            assert_allclose(
                0.25 - 0.25 * np.exp(-4 * d / 3),
                probs[non_diag_i, non_diag_j],
                atol=1e-7,
            )
...
def test_transition_probs_hky_expm(hky_params, branch_lengths, category_rates):
    ...
    probs_expm = transition_probs_expm(
        subst_model.q_norm(**hky_params), category_rates, branch_lengths
    )
...
def test_transition_prob_differential_tf_hky_kappa(
    hky_params, category_rates, inv_mult
):
    param_key = "kappa"
    branch_lengths = tf.convert_to_tensor(
        np.array([0.9, 2.2]), dtype=DEFAULT_FLOAT_DTYPE_TF
    )
    subst_model = HKY()
    params = hky_params
    with tf.GradientTape(persistent=True) as t:
        t.watch(params[param_key])
        q_norm = subst_model.q_norm(**params)
        transition_probs_vals = transition_probs_expm(
            q_norm, category_rates, branch_lengths
        )
    tf_jacs = t.jacobian(
        transition_probs_vals, params[param_key], experimental_use_pfor=False
    )
    ...
    diffs = transition_probs_differential(
        q_diffs, eigen, branch_lengths, category_rates, inv_mult=inv_mult
    )
...
@pytest.mark.parametrize("inv_mult", [False, True])
def test_transition_prob_differential_tf_hky_frequencies(
    hky_params, category_rates, inv_mult, freq_index
):
    branch_lengths = tf.convert_to_tensor(
        np.array([0.9, 2.2]), dtype=DEFAULT_FLOAT_DTYPE_TF
    )
    ...
    tf_diffs = (
        (transition_probs_inv @ tf_jacs[:, :, :, :, freq_index])
        if inv_mult
        else tf_jacs[:, :, :, :, freq_index]
    )
...
def test_transition_prob_differential_freq_num(hky_params, single_rates):
    ...
    q_diffs = subst_model.q_norm_frequency_differentials(**params)[freq_index]
    diffs = transition_probs_differential(
        q_diffs, eigen, branch_lengths, single_rates, inv_mult=False
    )
