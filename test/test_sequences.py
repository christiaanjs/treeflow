...
@pytest.mark.parametrize('branch_lengths,heights,parent_indices', [
...
def test_get_branch_lengths(branch_lengths, heights, parent_indices, function_mode):
    test_function = tf.function(treeflow.sequences.get_branch_lengths) if function_mode else treeflow.sequences.get_branch_lengths
    tree = treeflow.tree_processing.tree_to_tensor(dict(heights=heights, topology=dict(parent_indices=parent_indices)))
