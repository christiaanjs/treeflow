...
lognormal_dist = tfp.distributions.LogNormal(tf.convert_to_tensor(0.0, dtype=treeflow.DEFAULT_FLOAT_DTYPE_TF), tf.convert_to_tensor(1.0, dtype=treeflow.DEFAULT_FLOAT_DTYPE_TF))
...
def test_clock_approx_joint_sample_vector(hello_newick_file, clock_rate_approx, tree_statistic):
    tree_approx, _ = treeflow.model.construct_tree_approximation(hello_newick_file)
    tree = tree_approx.sample()
    approx_kwargs = dict(tree=tree, tree_statistic=tree_statistic) if clock_rate_approx == "scaled" else {}
    approx = tfp.distributions.JointDistributionNamed(dict(
        tree=tree_approx,
        clock_rate=treeflow.model.construct_distribution_approximation("q", "clock_rate", lognormal_dist, approx=clock_rate_approx, **approx_kwargs)[0]
    ))
    sample_shape = 2
    samples = approx.sample(sample_shape)
