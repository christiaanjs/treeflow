...
def test_coalescent_homochronous():
    ...
    sampling_times = tf.convert_to_tensor(np.array([0.0, 0.0, 0.0]))
    heights = tf.convert_to_tensor(np.array([1.0, 2.0]))
    parent_indices = tf.convert_to_tensor(np.array([3, 3, 4, 4]))
...
def test_coalescent_heterochronous(pop_size, expected):
    ...
    sampling_times = tf.convert_to_tensor(np.array([0.0, 0.1, 0.4, 0.0]))
    heights = tf.convert_to_tensor(np.array([0.2, 0.5, 0.8]))
    parent_indices = tf.convert_to_tensor(np.array([4, 4, 5, 6, 5, 6]))
    dist = ConstantCoalescent(pop_size, sampling_times)
    res = dist.log_prob(dict(heights=heights, topology=dict(parent_indices=parent_indices)))
    assert_allclose(res.numpy(), expected)