...
def test_hky_1cat_likelihood_beast(prep_likelihood, single_hky_params, single_rates, single_weights, hello_newick_file, hello_fasta_file):
    ...
    assert_allclose(tf_likelihood.compute_likelihood_from_partials(single_hky_params['frequencies'], single_weights).numpy(), -88.86355638556158, atol=1e-4)
...
def test_hky_1cat_beast_kappa_gradient(prep_likelihood, single_hky_params, single_rates, single_weights, hello_newick_file, hello_fasta_file):
    ...
    assert_allclose(tf_likelihood.compute_derivative(transition_prob_differential, single_weights).numpy(), 0.12373298571565322, atol=1e-4)
