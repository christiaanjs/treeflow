...
from treeflow.acceleration.native import tree_affine as native_tree_affine
...
def _ensure_native_built():
    ...
    if not os.path.exists(native_tree_affine.library_path()):
        try:
            build_tree_affine()
        except Exception as e:  # pragma: no cover - environment dependent
            unavailable(f"Could not build native tree affine op: {e}")
    try:
        native.load_op_library()
        native_ratio.load_op_library()
        native_tree_affine.load_op_library()
